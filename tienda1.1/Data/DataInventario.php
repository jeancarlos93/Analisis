...
function getInventario($tipoConsulta) {
    ...
    $tituloConsulta;
    if ($tipoConsulta == 4) {
        $sql = "select p.codigo,p.descripcion,marca,categoria,i.fecha,i.tipo,tp.talla, cp.color, if(i.tipo = 'entrada',p.precioUnitario*i.cantidad,p.precioVenta*i.cantidad) as total 
from inventario i 
...
where tipo = 'Salida';";
        ...
        $sql = "select inventario.codProducto,p.descripcion,(cantidadEntrada - cantidadSalida) as cantidad,cp.color,tp.talla,categoria,marca,p.precioVenta*(cantidadEntrada - cantidadSalida) as total
from inventario
